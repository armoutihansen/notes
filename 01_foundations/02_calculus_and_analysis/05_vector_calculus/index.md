---
layer: 01_foundations
type: index
status: evergreen
tags: []
created: 2026-03-02
---

# Vector Calculus Index

Navigation hub for vector calculus and geometric analysis.

## Notes

- [[vectors|Vectors]] — vector operations, norms, dot and cross products
- [[gradient_and_directional_derivative|Gradient and Directional Derivative]] — $\nabla f$, steepest ascent, level sets
- [[jacobian_and_hessian|Jacobian and Hessian]] — vector chain rule, curvature, second-order optimization

## Links
- [[01_foundations/02_calculus_and_analysis/index|Calculus and Analysis]]
- [[01_foundations/01_linear_algebra/index|Linear Algebra]]
- [[01_foundations/02_calculus_and_analysis/01_differentiation/index|Differentiation]]

---

**Navigation:** ← [[01_foundations/02_calculus_and_analysis/04_differential_equations/index|Differential Equations]] | [[01_foundations/02_calculus_and_analysis/index|Calculus & Analysis Index]]
