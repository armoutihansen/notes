...
At decoder step $t$, compute alignment score with each encoder state $a^{<t'>}$: $e_{t,t'} = \text{score}(s^{<t-1>}, a^{<t'>})$; common score functions: dot product, additive/Bahdanau: $v^\top \tanh(W_1 s + W_2 a)$, multiplicative: $s^\top W a$.

Attention weights (softmax): $\alpha_{t,t'} = \frac{\exp(e_{t,t'})}{\sum_{j} \exp(e_{t,j})}$; $\sum_{t'} \alpha_{t,t'} = 1$.

Context vector: $c^{<t>} = \sum_{t'} \alpha_{t,t'} a^{<t'>}$; fed to decoder along with previous output $y^{<t-1>}$.
